...
namespace MapConstants
{
    ...
    constexpr byte LEVEL_0 = 0;  // ✨ TUTORIAL: 1 room (NW only), simple gold
    constexpr byte LEVEL_1 = 1;  // 2 rooms accessible (NW, NE)
    constexpr byte LEVEL_2 = 2;  // 3 rooms accessible (NW, NE, SW)
    constexpr byte LEVEL_3 = 3;  // 4 rooms accessible (all) + explosives
    constexpr byte MIN_LEVEL = 0;
    constexpr byte MAX_LEVEL = 3;  // ✨ 4 nivele total acum!
}
...
enum class TileType : uint8_t
{
    ...
    BOMB  = 5,  // Explosive (visible only with light!) ← ACTIVAT!
    HIDDEN_GOLD = 6  // Gold hidden inside wall (revealed by explosion) 🎁
};
